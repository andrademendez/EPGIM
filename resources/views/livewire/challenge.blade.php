<x-content>
    ...
    <x-table.table>
        ...
        <x-table.tr>
            <x-table.td>{{ $camp->campania->user->name }}</x-table.td>
            <x-table.td>
                <div>
                    <h1 class="text-sm text-gray-700">{{ $camp->campania->title }}</h1>
                    <span class="text-xs text-gray-500">{{ $camp->campania->cliente->nombre }}</span>
                </div>
            </x-table.td>
            <x-table.td class="whitespace-nowrap">
                <div class="flex flex-col ">
                    <span class="text-sm text-gray-700">{{ $camp->campania->formatoMx($camp->campania->start) }}</span>
            ...
            <x-table.td class="whitespace-nowrap">
                ...
                <div>
                    <button class="text-orange-400 bg-red-200 mr-2 rounded-full p-1.5" title="En espera de documentos" wire:click="openPendiente({{ $camp->campania->id }})">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd" />
                        </svg>
                    </button>
                    <button class="text-green-600 bg-green-200 rounded-full p-1.5" wire:click="openConfirmar({{ $camp->campania->id }})" title="Confirmar campaña">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
